...
function fn_iniciar_sistema(){
    ...
    $(window).scroll(function(event) {
        ...
        if ( scrollPosicion > $("#menul_scroller").offset().top ){          
            ...
            $("#nav_flotante").addClass('fixed-top');
            $("#nav_flotante").removeClass('p-2').addClass('p-1');
            $('#menu-flotante').removeClass('d-none');
            $('#menu-estatico').addClass('d-none');
            if ( ! menuFlotante ){
                menuFlotante = true;
                $("#nav_flotante").hide();
                $("#nav_flotante").fadeIn('slow');
            }
        } else {
            menuFlotante = false;
            $("#form-inicio-sesion").removeClass('inicio-sesion-alt');
            $("#nav_flotante").removeClass('fixed-top');
            $("#nav_flotante").removeClass('p-1').addClass('p-2');
            $('#menu-flotante').addClass('d-none');
            $('#menu-estatico').removeClass('d-none');
        }
        contador();
    });
...
function contador() {
    ...
    counters.forEach(counter => {
        const updateCount = () => {
            ...
            const delay     = ( target < 500  )? 50 : ( target < 1000  )? 25 : 5;
            const inc       = ( target < 1000 )? 1  : target / speed;
            if ( count < target ) {
